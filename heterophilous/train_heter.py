...
import argparse
import random
from utils import NodeClassificationDataset
...
def train(g, feats, lr, epoch, device, lambda_loss, hidden_dim, sample_size=10,moving_average_decay=0.0):
    ...
    GNNModel = GNNStructEncoder(in_dim, hidden_dim, args.num_GNN, sample_size, lambda_loss=lambda_loss, tau = args.tau, lam = args.lam,  moving_average_decay=moving_average_decay, num_MLP=args.num_MLP)
    GNNModel.to(device)
...
def write_results(acc, best_epoch):
    ...
    f.write(args.dataset + ' --epochs ' + str(args.epoch_num) + ' --seed ' + str(args.seed) + ' --lr ' + str(args.lr) + ' --lambda_loss ' + str(args.lambda_loss) + ' --moving_average_decay ' + str(args.moving_average_decay) + ' --dimension ' + str(args.dimension) + ' --sample_size ' + str(args.sample_size) + ' --wd2 ' + str(args.wd2) + ' --num_MLP ' + str(args.num_MLP) + ' --num_GNN ' + str(args.num_GNN) + ' --tau ' + str(args.tau) + ' --lam ' + str(args.lam) + ' --best_epochs ' + " ".join(best_epoch) + f'   Final Test: {np.mean(acc):.4f} ± {np.std(acc):.4f}\n')
    f.close()
...
def train_new_datasets(dataset_str, epoch_num = 10, lr = 5e-6, lambda_loss=1, sample_size=10, hidden_dim=None,moving_average_decay=0.0):
    ...
    for index in range(10):
        ...
        split = utils2.DataSplit(dataset, split_lists[index]['train_idx'], split_lists[index]['valid_idx'], split_lists[index]['test_idx'], shuffle=True)

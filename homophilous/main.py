...
def write_results(acc):
    ...
    f.write(args.dataname + ' --epoch ' + str(args.epochs)+ ' --lr1 ' + str(args.lr1) + ' --lambda_loss ' + str(
        ...
        args.hid_dim) + ' --tau ' + str(args.temp)  + ' --lam ' + str(args.lam) + ' --num_MLP ' + str(args.num_MLP) + ' --n_layers ' + str(args.n_layers) + f'   Final Test: {np.mean(acc):.4f} ± {np.std(acc):.4f}\n')
    f.close()
...
if __name__ == '__main__':
    ...
    model = Model(in_dim, hid_dim, out_dim, n_layers, temp, args.use_mlp, args.moving_average_decay, args.num_MLP, args.lambda_loss, args.lam)
